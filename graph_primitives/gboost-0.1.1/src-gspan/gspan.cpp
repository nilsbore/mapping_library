...
void gSpan::mexAppendGraphIndices (Graph* g, unsigned int ID, double par,
	std::map<unsigned int, unsigned int>& counts, std::vector<unsigned int>& indices,
		std::vector<std::vector<int> >& node_indices)
{
	...
	ms->par = par;
	ms->counts = counts;	// copy
	ms->indices = indices;	// copy
	ms->node_indices = node_indices;	// copy
	
	for (int i = 0; i < node_indices.size(); ++i) {
	    for (std::vector<int>::iterator j = node_indices[i].begin(); j != node_indices[i].end(); ++j) {
	        std::cout << *j << " ";
	    }
	    std::cout << "\n" << std::endl;
	}
...
void gSpan::report (Projected &projected, unsigned int sup)
{
	...
	if (mex) {
		...
		std::map<unsigned int, unsigned int> counts;
		//std::vector<std::list<std::pair<int, int> > > edge_vector;
		...
		node_indices.resize(projected.size());
		...
		for (Projected::iterator cur = projected.begin(); cur != projected.end(); ++cur) {
			...
			mexPrintf("--------\n");
			PDFS* it = &(*cur);
			node_indices[counter].resize(g.size());
			int i = int(DFS_CODE.size()) - 1;
			while (it->prev != NULL) {
			    //mexPrintf("edge = %d, %d, %d, %u\n", it->edge->to, it->edge->from, it->edge->elabel, it->edge->id);
			    node_indices[counter][DFS_CODE[i].from] = it->edge->from + 1;
			    node_indices[counter][DFS_CODE[i].to] = it->edge->to + 1;
			    it = it->prev;
			    --i;
			}
			//mexPrintf("edge = %d, %d, %d, %u\n", it->edge->to, it->edge->from, it->edge->elabel, it->edge->id);
			node_indices[counter][DFS_CODE[i].from] = it->edge->from + 1;
			node_indices[counter][DFS_CODE[i].to] = it->edge->to + 1;
			//edge_vector[counter].push_back(std::pair<int, int>(it->edge->from, it->edge->to));
			indices[counter] = cur->id + 1; // one larger for index in matlab
			mexPrintf("--------\n");
			oid = cur->id;
			++counter;
		}
		mexAppendGraphIndices (&g, ID, sup, counts, indices, node_indices);
		++ID;
		return;
	}
		 ...
		 bool _directed)
{
	...
	if (nlhs >= 4) { // indices
	    plhs[3] = mxCreateCellMatrix(1, count);
	    assert (plhs[3] != NULL);
	}
	if (nlhs >= 5) { // node indices
	    plhs[4] = mxCreateCellMatrix(1, count);
	    assert (plhs[4] != NULL);
	}
		...
		cur = cur->next, ++idx)
	{
		...
		if (nlhs >= 4) { // indices
		    mxArray* tmp = mxCreateNumericMatrix(1, cur->indices.size(), mxUINT32_CLASS, 0);
		    indexP = (uint32_T *)mxGetPr(tmp);
		    for (unsigned ii = 0; ii < cur->indices.size(); ++ii) {
		        indexP[ii] = cur->indices[ii];
		    }
		    mxArray* cp = plhs[3];
		    mxSetCell(cp, idx, tmp);
		}
		
		if (nlhs >= 5) { // node indices
		    unsigned nn = cur->node_indices[0].size();
	        mxArray* tmp = mxCreateNumericMatrix(nn, cur->node_indices.size(), mxUINT32_CLASS, 0);
		    indexP = (uint32_T *)mxGetPr(tmp);
		    for (unsigned ii = 0; ii < cur->node_indices.size(); ++ii) {
		        for (unsigned jj = 0; jj < nn; ++jj) {
		            indexP[ii*nn + jj] = cur->node_indices[ii][jj];
		        }
		    }
		    mxArray* cp = plhs[4];
		    mxSetCell(cp, idx, tmp);
