...
public:
	 ...
	 */
	bool toGraph (Graph &);
	//bool toGraphIndices (Graph &g, std::vector<int>&);
...
private:
	...
	void mexAppendGraph (Graph* g, unsigned int ID, double par,
		std::map<unsigned int, unsigned int>& counts);
    void mexAppendGraphIndices (Graph* g, unsigned int ID, double par,
		std::map<unsigned int, unsigned int>& counts, std::vector<unsigned int>&,
