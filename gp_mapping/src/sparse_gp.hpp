...
template <class Kernel, class Noise>
double sparse_gp<Kernel, Noise>::log_likelihood(const VectorXd& X, double y)
{
...
template <class Kernel, class Noise>
void sparse_gp<Kernel, Noise>::compute_log_likelihoods(VectorXd& l, const MatrixXd& X, const VectorXd& y)
{
    l.resize(X.rows());
    for (int i = 0; i < X.rows(); ++i) {
        l(i) = log_likelihood(X.row(i).transpose(), y(i));
    }
}
...
void sparse_gp<Kernel, Noise>::likelihood_dtheta(VectorXd& dtheta, const Vector2d& x, double y)
{
    ...
    MatrixXd kstar_dtheta;
    kernel.kernel_dtheta(kstar_dtheta, x, x);
    VectorXd sigma_dtheta(kernel.param_size());
    sigma_dtheta = kstar_dtheta.transpose() + 2.0f*k_dtheta.transpose()*C*k;
    double offset = y - alpha.transpose()*k;
    double sigma = s20 + kstar + k.transpose()*C*k;
    VectorXd first = -sigma_dtheta;
    //std::cout << "Kstar: " << kstar_dtheta.rows() << ", " << kstar_dtheta.cols() << std::endl;
    //std::cout << "alpha: " << alpha.rows() << ", " << alpha.cols() << std::endl;
    VectorXd second = 2.0f*offset*k_dtheta.transpose()*alpha;
    VectorXd third = sigma_dtheta/sigma*offset*offset;
    dtheta = (first + second + third)/sigma;
}
